...
class CheckUser(commands.Cog, name='Staff'):
    ...
    async def discord_id(self, ctx, discord_user: discord.User = ''):
        ...
        discord_usernames = ''
        for discord_id in player_data.get('discord_ids', {}):
            discord_usernames += f'{discord_id["discord_usernames"][-1]["discord_username"]} ({discord_id["discord_id"]})\n'

        mojang_usernames = ''
        for mojang_uuid in player_data.get('mojang_uuids', {}):
            mojang_usernames += f'{mojang_uuid["mojang_usernames"][-1]["mojang_username"]} ({mojang_uuid["mojang_uuid"]})\n'
