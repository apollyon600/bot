...
def pigman(player, stats, pet):
	...
	if player.weapon == 'PIGMAN_SWORD':
		...
		stats['weapon strength'] += int(pet.level * 0.25)
...
def yeti(player, stats, pet):
	...
	if pet.rarity == 'legendary' and player.weapon == 'YETI_SWORD':
		stats['damage'] += pet.level
		stats['intelligence'] += pet.level
...
pet_stats = {
	...
	'GHOUL': {
		...
		'stats': {
			'health': lambda lvl: lvl,
			'intelligence': lambda lvl: lvl * 7 // 10
		},
		'ability': none,
		'type': 'combat',
	...
	'BABY_YETI': {
		...
		'stats': {
			...
			'intelligence': lambda lvl: lvl * 3 // 4
		},
		'type': 'fishing',
		'ability': yeti,
